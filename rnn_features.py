...
def get_vector(model, word_arr, index, con_win, word_dim, normalize, one_way, gen_vec, remove_word, unk, unk_token):
    ...
    if one_way == 0:
        try:
            ...
            left_win = con_win
            right_win = con_win
        min_index = index - left_win
        min_index = max(min_index,0)
        max_index = index + right_win + 1
...
def append_features(model, word, word_dim, gen_vec, unk, unknown_token):
    ...
    if word.__contains__('@'):
        anomaly = word.index('@')
        word = word[:anomaly] + word[anomaly + 1:]
    if word != 'NUMBER' and word != 'URL' and not word.__contains__('http'): # and not string.digits.__contains__(word):
...
def extra_word_features(word_arr, j):
    ...
    # Is a number
    num = 0
    if string.digits.__contains__(word_arr[j][0]):
        num = 1
    feature.append(num)
...
def word_feature(word_arr, model, window, feat_window, i, context, word_dim, gen_vec, unk, unknown_token, extra_feats = 1):
    feature = []
    if context == 1:
        try:
...
def window_feature(array, array_set, window, i, context, status = 0):
    feature = []
    ...
    if context == 1:
        ...
        for j in range(s, e):
            if j < start or j >= end or status == 1:
...
def get_phrase(model, lemma_arr, i, word_dim):
    temp = np.zeros(word_dim)
    feat = [0,0]
    if i != lemma_arr.__len__() - 1:
        phrase = lemma_arr[i] + '_' + lemma_arr[i + 1]
        if model.__contains__(phrase):
            temp = model[phrase]
            feat[0] = 1
        else:
            phrase = lemma_arr[i].lower() + '_' + lemma_arr[i + 1].lower()
            if model.__contains__(phrase):
                temp = model[phrase]
                feat[0] = 1
            else:
                capital1 = lemma_arr[i][0].upper()
                phrase = capital1 + lemma_arr[i][1:] + '_' + lemma_arr[i + 1]
                if model.__contains__(phrase):
                    temp = model[phrase]
                    feat[0] = 1
                else:
                    capital2 = lemma_arr[i + 1][0].upper()
                    phrase = lemma_arr[i] + '_' + capital2 + lemma_arr[i + 1][1:]
                    if model.__contains__(phrase):
                        temp = model[phrase]
                        feat[0] = 1
                    else:
                        phrase = capital1 + lemma_arr[i][1:] + '_' + capital2 + lemma_arr[i + 1][1:]
                        if model.__contains__(phrase):
                            temp = model[phrase]
                            feat[0] = 1
                        else:
                            phrase = lemma_arr[i].upper() + '_' + lemma_arr[i + 1].upper()
                            if model.__contains__(phrase):
                                temp = model[phrase]
                                feat[0] = 1
                            else:
                                phrase = lemma_arr[i] + lemma_arr[i + 1]
                                if model.__contains__(phrase):
                                    temp = model[phrase]
                                    feat[1] = 1
                                else:
                                    phrase = lemma_arr[i].lower() + lemma_arr[i + 1].lower()
                                    if model.__contains__(phrase):
                                        temp = model[phrase]
                                        feat[1] = 1
                                    else:
                                        phrase = capital1 + lemma_arr[i][1:] + lemma_arr[i + 1]
                                        if model.__contains__(phrase):
                                            temp = model[phrase]
                                            feat[1] = 1
                                        else:
                                            phrase = lemma_arr[i] + capital2 + lemma_arr[i + 1][1:]
                                            if model.__contains__(phrase):
                                                temp = model[phrase]
                                                feat[1] = 1
                                            else:
                                                phrase = capital1 + lemma_arr[i][1:] + capital2 + lemma_arr[i + 1][1:]
                                                if model.__contains__(phrase):
                                                    temp = model[phrase]
                                                    feat[1] = 1
                                                else:
                                                    phrase = lemma_arr[i].upper() + lemma_arr[i + 1].upper()
                                                    if model.__contains__(phrase):
                                                        temp = model[phrase]
                                                        feat[1] = 1
    #feat = [0, 0]
...
def populate_features(word_dim, token_arr, lemma_arr, pos_arr, tag_arr, token_features, lemma_features,
                     ...
                     con_win, status, forward_pass, backward_pass, forward_reverse, backward_reverse):

    forward_features = []
    backward_features = []
    for j in range(0, token_arr.__len__()):
        feature_set = []
        #Forward Pass
        if forward_pass == 1:
            for i in range(0,j+1):
                feature = []
                if tag_features == 1:
                    temp = window_feature(tag_arr, tag_set, tag_window, i, 0, status)
                ...
                if token_features == 1:
                    ...
                    feature = np.concatenate((feature, temp), axis=0)
                    #temp = get_phrase(model, token_arr, i, word_dim) #Different levels of detection get a separate binary feature?
                    #feature = np.concatenate((feature, temp), axis=0)
                    #temp = np.zeros(word_dim)
                    #feature = np.concatenate((feature, temp), axis=0)
                    #temp = get_vector(model, token_arr, i, [0,1], word_dim, 0, 0, gen_vec, -1, 0, unknown_token)
                    #temp = np.zeros(word_dim)
                    #feature = np.concatenate((feature, temp), axis=0)
                ...
                if lemma_features == 1:
                    ...
                    feature = np.concatenate((feature, temp), axis=0)
                    #temp = get_phrase(model, lemma_arr, i, word_dim)
                    #feature = np.concatenate((feature, temp), axis=0)

                    #temp = np.zeros(word_dim)
                    #feature = np.concatenate((feature, temp), axis=0)
                    #temp = get_vector(model, lemma_arr, i, [0, 1], word_dim, 0, 0, gen_vec, -1, 0, unknown_token)
                    #temp = np.zeros(word_dim)
                    #feature = np.concatenate((feature, temp), axis=0)
            ...
            if forward_reverse == 1:
                feature_set.reverse()
            forward_features.append(feature_set)
        if backward_pass == 1:
            for i in range(j, token_arr.__len__()):
                feature = []
                if tag_features == 1:
                    temp = window_feature(tag_arr, tag_set, tag_window, i, 0, status)
                    feature = np.concatenate((feature, temp), axis=0)
                if token_features == 1:
                    temp = word_feature(token_arr, model, window, feat_window, i, context, word_dim, gen_vec, unk,
                                        unknown_token)
                    feature = np.concatenate((feature, temp), axis=0)
                    # temp = get_phrase(model, token_arr, i, word_dim) #Different levels of detection get a separate binary feature?
                    # feature = np.concatenate((feature, temp), axis=0)
                    ...
                    # print "temp: " + str(temp.__len__())
                if lemma_features == 1:
                    temp = word_feature(lemma_arr, model, window, feat_window, i, context, word_dim, gen_vec, unk,
                                        unknown_token)
                    feature = np.concatenate((feature, temp), axis=0)
                    # temp = get_phrase(model, lemma_arr, i, word_dim)
                    # feature = np.concatenate((feature, temp), axis=0)
                ...
                if pos_features == 1:
                    ...
                    # print "In tag_features: " + str(feature.__len__())
                    # print "temp: " + str(temp.__len__())
                temp = context2vec(avg_tokens, remove, model, token_arr, word_dim, normalize, sentence_embedding, gen_vec,
                                   i, uni_skip,
                                   bi_skip, skipthoughts_tokens_vectors, sentence_counter, con_win, unk, unknown_token)
                feature = np.concatenate((feature, temp), axis=0)
                # print "In token_context: " + str(feature.__len__())
                # print "temp: " + str(temp.__len__())
                temp = context2vec(avg_lemmas, remove, model, lemma_arr, word_dim, normalize, sentence_embedding, gen_vec,
                                   i, uni_skip,
                                   bi_skip, skipthoughts_lemmas_vectors, sentence_counter, con_win, unk, unknown_token)
                feature = np.concatenate((feature, temp), axis=0)
                # print "In lemma_context: " + str(feature.__len__())
                ...
                # print "temp: " + str(temp_features.__len__())
                feature_set.append(feature)
            if backward_reverse == 1:
                feature_set.reverse()
            backward_features.append(feature_set)
    return forward_features, backward_features

def rnn_features(path, window, feat_window, pos_window, tag_window, mwe_window, word_dim, tag_set, model, status, ngram,
                ...
                separate_lexicons,  pos_tags, emission_arr, poshmm, alpha, sentence_embedding, gen_vec, unk, unknown_token,
                remove, forward_pass, backward_pass, forward_reverse, backward_reverse,
                skipthoughts_tokens_path = None, skipthoughts_lemmas_path = None, uni_skip = 0, bi_skip = 0):

    forward_training_features = []
    backward_training_features = []
    training_labels = []
    features = []
    ...
    while line != '':
        if line != '\n':
            ...
            sentence_counter += 1
            #print sentence_counter
            forward_features, backward_features = populate_features(word_dim, token_arr, lemma_arr, pos_arr, tag_arr, token_features, lemma_features,
                     pos_features, tag_features, context, window, feat_window, pos_window, tag_window, mwe_window, gen_vec, unk, unknown_token,
                     avg_tokens, avg_lemmas, normalize, in_mwe, tag_distribution, pos_tags, remove, model, sentence_embedding,
                     uni_skip, bi_skip, skipthoughts_tokens_vectors, skipthoughts_lemmas_vectors, sentence_counter,
                     con_win, status, forward_pass, backward_pass, forward_reverse, backward_reverse)
            for feature in forward_features:
                # if feature_size == 0:
                #     feature_size = feature[0].__len__()
                #     print "There are " + str(feature_size) + " features in total"
                # else:
                #     if feature[0].__len__() != feature_size:
                #         print "Standard feature is " + str(feature_size) + " but encoutered " + str(feature.__len__())
                #         assert feature[0].__len__() == feature_size
                forward_training_features.append(feature)
                if backward_pass != 1:
                    feature_count += 1
            ...
            for feature in backward_features:
                ...
                #         print "Standard feature is " + str(feature_size) + " but encoutered " + str(feature.__len__())
                #         assert feature[0].__len__() == feature_size
                backward_training_features.append(feature)
                feature_count += 1
                if feature_count % 10000 == 0:
                    print "Extracted " + str(feature_count) + " features"
            token_arr = []
            ...
            tag_arr = []
        line = training_file.readline()
    print "There are " + str(forward_training_features.__len__()) + " forward features"
    print "There are " + str(backward_training_features.__len__()) + " backward features"
    ...
    return [forward_training_features,backward_training_features], training_labels
