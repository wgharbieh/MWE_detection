...
import extract_mwes
import sys
import ktuple_features as features
import skipthoughts
import operator
...
def get_pred(test_file,validation_features,sess,x,W,b,y,model = None,final = 0):
    ...
    error_analysis.analyze_pair(token_arr, pos_arr, 10, validation_labels, predictions, tag_set)
    error_analysis.analyze_branches(token_arr, pos_arr, )
...
def get_classes(emission_arr, k_tuple):
    mapping_dict = {}
    i = 0
    for key in emission_arr:
        if key[0] != start_tag and key[k_tuple-1] != end_tag:
            mapping_dict[i] = key
            i += 1
    return mapping_dict
    

def train_NN(training_features,training_labels,validation_features, validation_labels, k_tuple, emission_arr, 
            cost_fun, learning_rate, stop, predict, rho, batch_size, test_file, model = None, hidden = None):
    acc_train = 0
    acc_val = 0
    ...
    W = []
    b = []
    class_dict = get_classes(emission_arr, k_tuple)
    classes = class_dict.__len__()
    ...
    for i in range(0, num):
        ...
        for j in range(0,splits):
            if test == 1:
                ...
                WINDOW, POS_WINDOW, TAG_WINDOW, MWE_WINDOW, DIM, tag_set, model,0, ngram, tag_features,
            ...
            if test == 1:
                ...
                WINDOW, POS_WINDOW, TAG_WINDOW, MWE_WINDOW, DIM, tag_set, model,1, ngram, tag_features,
                token_features, lemma_features, pos_features, sub_token_lemma, context, con_win, avg_tokens, avg_lemmas,
                normalize, debug_features, in_mwe, tag_distribution, separate_lexicons,  pos_tags, emission_arr, poshmm, reg,
                sentence_embedding, gen_vec, unk, unknown_token, remove_target, TOKENS_PATH + "fold_" + str(i+1) + str(j+1) + "_train_vectors",
                LEMMAS_PATH + "fold_" + str(i+1) + str(j+1) + "_train_vectors", uni_skip, bi_skip)
            ...
            for rep in range(0,repetitions):
                ...
                if test == 1:
                    ...
                    validation_labels, ngram, emission_arr, cost_function, learning_rate, max_iter, predict, 
                    rho, batch_size, PATH + "dimsum16.test.blind", model, hidden)
                elif sample == 1:
                    train_score, val_score = train_NN(training_features,training_labels,validation_features,
                    validation_labels, ngram, emission_arr, cost_function, learning_rate, max_iter, predict, 
                    rho, batch_size, PATH + "fold_test", model, hidden)
                else:
                    train_score, val_score = train_NN(training_features,training_labels,validation_features,
                    validation_labels, ngram, emission_arr, cost_function, learning_rate, max_iter, predict, 
                    rho, batch_size, PATH + "fold_" + str(i+1) + str(j+1) + "_test", model, hidden)
