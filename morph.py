...
def stem(w, p):
    ...
    if w == 'fell' and p == 'VBD':
        return 'fall'
    elif w == 'found' and p in {'VBD', 'VBN'}:
        return 'find'
    elif w == 'lay' and p == 'VBD':
        return 'lie'
    elif w == 'saw' and p == 'VBD':
        return 'see'
    elif w == 'people' and p == 'NNS':
        return 'person'

    if w == 'cannot' or "'" in w:
        tt = word_tokenize(w + ' .')[
             :-1]  # period ensures no part of the word is interpreted as sentence-final punctuation
    else:
        tt = [w]
...
def loadMorphDataOriginalFormat():
    ...
    global morphMap
    morphMap = {}
    with gzip.open(_options.get('morphFile',
                                os.path.dirname(os.path.abspath(__file__)) + '/../data/oldgaz/MORPH_CACHE.gz')) as inF:
        for ln in inF:
...
def loadMorphDataNewFormat():
    ...
    morphMap = {}
    with gzip.open(_options.get('morphFile',
                                os.path.dirname(os.path.abspath(__file__)) + '/../data/morph/MORPH_CACHE.gz')) as inF:
        for ln in inF:
            pos, w, stemS = ln[:-1].split('\t')
            addMorph(w, pos, stemS)
            addMorph(w, 'UNKNOWN', stemS)
